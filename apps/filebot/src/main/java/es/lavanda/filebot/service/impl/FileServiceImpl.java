...
//    }
    ...
    public boolean isValidForFilebot(String fileOrDirectory) {
        ...
        if (!file.exists()) {
            throw new FilebotException(String.format("File not found %s", fileOrDirectory));
        }
        if (file.isFile()) {
    ...
    @Override
    public List<FilebotExecution.FileExecutor> getFilesExecutor(String path) {
        List<FilebotExecution.FileExecutor> files = new ArrayList<>();
