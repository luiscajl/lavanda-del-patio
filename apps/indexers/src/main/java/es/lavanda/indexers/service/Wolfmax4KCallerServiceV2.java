...
public class Wolfmax4KCallerServiceV2 {
    ...
    public List<Index> getIndexForMainPageV2(String url, Index.Type type, Index.Quality quality, boolean multipleOnAPage) {
        ...
        Document document = retryGetHtmlResponse(url);
    ...
    private Document retryGetHtmlResponse(String url) {
        FlaresolverrIDTO flaresolverrIDTOWolfmax4k;
        Document document;
        do {
            flaresolverrIDTOWolfmax4k = getHtmlResponse(url);
            document = Jsoup.parse(flaresolverrIDTOWolfmax4k.getSolution().getResponse());
        } while (document == null);
        return document;
    }

    private List<Index> getNewReleasesV2(Document document, Index.Type type, Index.Quality quality, boolean multipleOnAPage) {
        Elements elements = document.getElementsByClass("col-lg-2");
        if (elements == null || elements.isEmpty()) {
            log.error("No elements found for class col-lg-2");
            return Collections.emptyList();
        }
        return new ArrayList<>(getIndexFromColLg2(elements, type, quality, multipleOnAPage));
    }
    ...
    private Index buildIndex(Element colLg2, Index.Type type, Index.Quality quality, String url) {
        ...
        Document documentIndividual = retryGetHtmlResponse(index.getUrl());
    ...
    private List<Index> buildIndexForMulti(Element colLg2, Index.Type type, Index.Quality quality, String url) {
        ...
        Document documentMainShow = retryGetHtmlResponse(urlMainShow);
        Element rowAfterLayoutSection = documentMainShow.getElementsByClass("row gx-lg-4 gx-0").first();
        if (rowAfterLayoutSection == null || rowAfterLayoutSection.getElementsByClass("temp-serie") == null) {
            indexes.add(buildIndex(colLg2, type, quality, url));
            return indexes;
        }
        String imageElement = colLg2.getElementsByClass("img-fluid rounded-1").attr("src");
        String imageUrl = getImageUrl(imageElement);
